...
The generator will create a single `.scala` file for each `.proto` file. This
file will define a single Scala object that will contain classes that
correspond to the content of the proto file.

That object name is determined by converting the name of the proto file
to upper case and adding "PB" to it. If the proto file defines a
"java_outer_classname" option, then the object name will be that option with
"PB" added to it.

The generated file will have be in the Scala package specified by the
`package` keyword in the proto file, unless a `java_package` option has been
specified.
...
case class Order(..., paymentType: Payment.PaymentType) {
    ...
    // Sets the PaymentType to NotSet
    def clearPaymentType: Order
}

object Order {
    sealed trait PaymentType {
        def isNotSet: Boolean
    ...
    case object NotSet extends PaymentType
...
o3.paymentType match {
    ...
    case PaymentType.NotSet =>  // handle exceptional case...
}
