...
class StructUtilsSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "fromStruct of empty message" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map.empty)).right.value must be(
      TestAllTypes()
    )
  }
  ...
  "fromStruct with int field" should "pass" in {
    val someIntValue = Random.nextInt()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int32" -> Value(Value.Kind.NumberValue(someIntValue.toDouble))))).right.value must be(
  ...
  "fromStruct with long field" should "pass" in {
    val someLongValue = Random.nextLong()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int64" -> Value(Value.Kind.StringValue(someLongValue.toString))))).right.value must be(
      TestAllTypes().withOptionalInt64(someLongValue)
    )
  }
  "fromStruct with double field" should "pass" in {
    val someDoubleValue = Random.nextDouble()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_double" -> Value(Value.Kind.NumberValue(someDoubleValue.toDouble))))).right.value must be(
  ...
  "fromStruct with float field" should "pass" in {
    val someFloatValue = Random.nextFloat()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_float" -> Value(Value.Kind.NumberValue(someFloatValue.toDouble))))).right.value must be(
  ...
  "fromStruct with string field" should "pass" in {
    val someStringValue = Random.alphanumeric.take(Random.nextInt(500)).mkString
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_string" -> Value(Value.Kind.StringValue(someStringValue))))).right.value must be(
  ...
  "fromStruct with byte string field" should "pass" in {
    val someBytesValue = bytes(0xe3, 0x81, 0x82)
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_bytes" -> Value(Value.Kind.StringValue(someBytesValue.toStringUtf8))))).right.value must be(
  ...
  "fromStruct with boolean field" should "pass" in {
    val someBooleanValue = Random.nextBoolean()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_bool" -> Value(Value.Kind.BoolValue(someBooleanValue))))).right.value must be(
      TestAllTypes().withOptionalBool(someBooleanValue)
    )
  }

  "fromStruct with enum field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_foreign_enum" -> Value(Value.Kind.StringValue(ForeignEnum.FOREIGN_BAR.name))))).right.value must be(
  ...
  "fromStruct with repeated field" should "pass" in {
    val someRepeatedIntValue = Seq(Random.nextInt(), Random.nextInt())
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("repeated_int32" -> Value(Value.Kind.ListValue(ListValue(someRepeatedIntValue.map(i => Value(Value.Kind.NumberValue(i.toDouble))))))))).right.value must be(
      TestAllTypes().withRepeatedInt32(someRepeatedIntValue)
    )
  }

  "fromStruct with empty nested message field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_nested_message" -> Value(Value.Kind.StructValue(Struct(Map.empty)))))).right.value must be(
  ...
  "fromStruct with non-empty nested message field" should "pass" in {
    val someIntValue = Random.nextInt()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_nested_message" -> Value(Value.Kind.StructValue(Struct(Map("bb" -> Value(Value.Kind.NumberValue(someIntValue.toDouble))))))))).right.value must be(
  ...
  "fromStruct with empty import message field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_import_message" -> Value(Value.Kind.StructValue(Struct(Map.empty)))))).right.value must be(
      TestAllTypes().withOptionalImportMessage(ImportMessage())
    )
  }

  "fromStruct with empty foreign message field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_foreign_message" -> Value(Value.Kind.StructValue(Struct(Map.empty)))))).right.value must be(
  ...
  "ser-deser of empty message" should "pass" in {
    ...
    StructUtils.fromStruct(TestAllTypes.messageCompanion, StructUtils.toStruct(types)).right.value must be(types)
  }
  ...
  "fromStruct with two level deep message" should "pass" in {
    ...
    StructUtils.fromStruct(ContainerMessage.messageCompanion, StructUtils.toStruct(types)).right.value must be(types)
  }
  ...
  "fromStruct with missing enum field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_foreign_enum" -> Value(Value.Kind.StringValue("non_existent"))))).left.value must be(
  ...
  "fromStruct with faulty int field" should "pass" in {
    ...
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int32" -> Value(Value.Kind.NumberValue(someFaultyInt))))).left.value must be(
  ...
  "fromStruct with faulty long field" should "pass" in {
    ...
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int64" -> Value(Value.Kind.StringValue(someFaultyLong))))).left.value must be(
  ...
  "fromStruct with int field value for string field key" should "pass" in {
    ...
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_string" -> Value(Value.Kind.NumberValue(someFaultyString))))).left.value must be(
  ...
  "fromStruct with boolean field value for string field key" should "pass" in {
    val someBooleanValue = Random.nextBoolean()
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_string" -> Value(Value.Kind.BoolValue(someBooleanValue))))).left.value must be(
  ...
  "fromStruct with message field value for string field key" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_string" -> Value(Value.Kind.StructValue(Struct(Map.empty)))))).left.value must be(
  ...
  "fromStruct with repeated field value for single field key" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_string" -> Value(Value.Kind.ListValue(ListValue(Seq(Value(Value.Kind.StringValue("Hi"))))))))).left.value must be(
  ...
  "fromStruct with string field value for int field key" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int32" -> Value(Value.Kind.StringValue("Hi"))))).left.value must be(
  ...
  "fromStruct with empty optional int field" should "pass" in {
    StructUtils.fromStruct(TestAllTypes.messageCompanion, Struct(Map("optional_int32" -> Value(Value.Kind.Empty)))).right.value must be(
      TestAllTypes()
    )
  }
}
