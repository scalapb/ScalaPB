...
class CustomOptionsSpec extends AnyFlatSpec with Matchers with OptionValues {
  ...
  "Options existing" should "return Some(option)" in {
    fooOptions.extension(CustomOptionsProto.messageB).value must be(
      MessageB(b = Some("BBB"), c = Some("CCC"), d = Seq("D1", "D2"))
    )
  ...
  "Repeated list of messages" should "work when empty" in {
    fooOptions.extension(CustomOptionsProto.repMessageC) must be(Seq.empty)
  ...
  "setting repeated fields" should "work" in {
    ...
    validateSetter(CustomOptionsProto.repBytes)(
      Seq(ByteString.copyFromUtf8("foo"), ByteString.copyFromUtf8("bar"))
    )
  ...
  "Custom name on field descriptor" should "translate to custom type" in {
    ...
    CustomOptionsProto.repName.get(field2Opts) must be(
      Seq(FullName("", "Doe"), FullName("Moe", ""))
    )
  }
  ...
  "packed fields" should "parse correctly" in {
    ...
    m.extension(CustomOptionsProto.packedEnum) must be(
      Seq(GoodOrBad.BAD, GoodOrBad.GOOD, GoodOrBad.Unrecognized(39))
    )
  ...
  "setting packed field" should "work correctly" in {
    ...
    validateSetter(CustomOptionsProto.packedEnum)(
      Seq(GoodOrBad.BAD, GoodOrBad.GOOD, GoodOrBad.Unrecognized(39))
    )
