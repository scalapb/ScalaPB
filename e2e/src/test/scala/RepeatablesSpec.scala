...
class RepeatablesSpec extends AnyFlatSpec with ScalaCheckDrivenPropertyChecks with Matchers {
  ...
  "with" should "set the entire list" in {
    forAll(repGen, repGen) { (rep, other) =>
      rep
  ...
  "addAll" should "add the entire list" in {
    forAll(repGen, repGen) { (rep, other) =>
      rep
        ...
        .addAllNesteds(other.nesteds) must be(mergeRepeatables(rep, other))
    }
  }
  ...
  "add" should "add one or two elements" in {
    forAll(repGen, Arbitrary.arbitrary[Int], Arbitrary.arbitrary[Int]) { (rep, int1, int2) =>
  ...
  "update" should "add one element" in {
    forAll(repGen, Arbitrary.arbitrary[Int]) { (rep, int1) =>
      rep.update(_.ints :+= int1) must be(rep.copy(ints = rep.ints :+ int1))
    }
  }

  "update" should "update one element in place" in {
    forAll(repGen.suchThat(_.ints.size >= 2), Arbitrary.arbitrary[Int]) { (rep, int1) =>
      rep.update(_.ints(1) := int1) must be(rep.copy(ints = rep.ints.updated(1, int1)))
    }
    forAll(repGen.suchThat(_.nesteds.size >= 2), Arbitrary.arbitrary[Int]) { (rep, int1) =>
  ...
  "concatenate message" should "result in merged messages" in {
    forAll(repGen, repGen) { (rep1, rep2) =>
  ...
  "parsing delimited stream" should "be the inverse of writing delimited stream" in {
    forAll(Gen.listOf(repGen)) { list =>
      ...
      val parsedInputStream = {
        ...
        Stream
          .continually(RepeatablesTest.parseDelimitedFrom(is))
      ...
      val parsedCoded = {
        ...
        Stream
          .continually(RepeatablesTest.parseDelimitedFrom(cis))
          .takeWhile(_.isDefined)
          .map(_.get)
          .toList
      }
