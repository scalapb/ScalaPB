...
class Printer(singleLineMode: Boolean) {
  ...
  def printField(fd: FieldDescriptor, value: Any, out: TextGenerator) = {
    ...
    } else if (fd.isOptional) value.asInstanceOf[Option[Any]].foreach {
      v: Any => printSingleField(fd, v, out)
    } else {
    ...
    fd.getType match {
      ...
      case Type.STRING =>
        ...
        out
          ...
          .add(
            if (escapeNonAscii)
              TextFormatUtils.escapeText(value.asInstanceOf[String])
            else
              TextFormatUtils.escapeDoubleQuotesAndBackslashes(value.asInstanceOf[String]))
          .add("\"")
