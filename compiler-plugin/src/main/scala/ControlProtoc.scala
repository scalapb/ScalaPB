...
import com.google.protobuf.Descriptors.{FieldDescriptor, EnumDescriptor, Descriptor, FileDescriptor}
...
class ProtobufGenerator(request: CodeGeneratorRequest) {
...
object ControlProtoc extends App {
  ...
  def generateCode(request: CodeGeneratorRequest): CodeGeneratorResponse = {
    val generator = new ProtobufGenerator(request)
    ...
    request.getFileToGenerateList.foreach {
      name =>
        ...
        val responseFile = generator.generateFile(file)
