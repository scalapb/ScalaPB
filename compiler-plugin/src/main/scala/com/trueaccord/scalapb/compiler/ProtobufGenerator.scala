...
object ProtobufGenerator {
  ...
  def printMessage(message: Descriptor,
    ...
    val descriptorGetter = if (message.getContainingType != null)
      fullScalaName(message.getContainingType) + s".descriptor.getNestedType(${message.getIndex})"
    else
      scalaFullOuterObjectName(message.getFile) + s".descriptor.getMessageType(${message.getIndex})"
    ...
    .add(s"def fromJavaProto(javaPbSource: $myFullJavaName): $myFullScalaName = $myFullScalaName(")
      ...
      .outdent
      .add(")")
      .add(s"lazy val descriptor: com.google.protobuf.DescriptorProtos.DescriptorProto = $descriptorGetter")
  ...
  def generateFile(file: FileDescriptor): CodeGeneratorResponse.File = {
    ...
    val protoAsString = escapeString(new String(protoChars.map(_.toChar)))
    ...
    val p1 = p0.add(
      ...
      .add("lazy val descriptor = com.google.protobuf.DescriptorProtos.FileDescriptorProto.parseFrom(")
      .indent
      .add(protoAsString + ".toCharArray.map(_.toByte))")
      .outdent
      .outdent
      .add("}")
