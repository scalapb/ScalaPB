...
class ProtobufGenerator(val params: GeneratorParams) extends DescriptorPimps {
  ...
  def javaFieldToScala(javaHazzer: String, javaGetter: String, field: FieldDescriptor): String = {
    ...
    if (field.supportsPresence)
      ...
      valueConversion(s"${javaGetter}.asScala.to[${field.collectionType}]", tpe = ExpressionBuilder.Collection)
      ...
      .print(message.fields) {
        case (printer, field) =>
          ...
          printer
            ...
            val emptyValue = if (field.isMap) "scala.collection.immutable.Map.empty" else field.collectionType + ".empty"
            p.addStringMargin(
