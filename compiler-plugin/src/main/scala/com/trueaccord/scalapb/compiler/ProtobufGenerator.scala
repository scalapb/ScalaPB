...
class ProtobufGenerator(params: GeneratorParams) {
  ...
  def generateInternalFields(message: Descriptor, fp: FunctionalPrinter): FunctionalPrinter = {
    ...
    fp.add(s"""case "${message.scalaTypeName}" => Seq(${message.getFields.map(makeDescriptor).mkString(", ")})""")
  ...
  def scalaFileHeader(packageName: String): FunctionalPrinter = {
    new FunctionalPrinter().addM(
      s"""// Generated by the Scala Plugin for the Protocol Buffer Compiler.
         |// Do not edit!
         |
         |${if (packageName.nonEmpty) ("package " + packageName) else ""}
         |
