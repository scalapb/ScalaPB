...
private[compiler] class ParseFromGenerator(
  ...
  def defParseFrom(myFullScalaName: String)(f: (FunctionalPrinter => FunctionalPrinter), fs: (FunctionalPrinter => FunctionalPrinter)*)(printer: FunctionalPrinter): FunctionalPrinter = printer
    .add(
      s"def parseFrom(`_input__`: _root_.com.google.protobuf.CodedInputStream): $myFullScalaName = {"
    ).indented(fs.foldLeft(f)(_ andThen _))
    .add("}")

  def varRequiredFields(requiredFieldMap: Map[FieldDescriptor, Int]): PrinterEndo = 
  ...
  def varFields(fields: Seq[Field]): PrinterEndo = _.print(fields)((fp, f) =>
    fp.add(
      s"${if (f.isRepeated) "val" else "var"} ${f.name}: ${f.typeName} = ${f.default}"
    )
  )

  def parseLoop(fields: Seq[FieldDescriptor]): PrinterEndo = {
    _.add("var _done__ = false")
      .add("while (!_done__) {")
      .indented(
        _.add("val _tag__ = _input__.readTag()")
          .add("_tag__ match {")
          .indented(
            _.add("case 0 => _done__ = true")
  ...
  def checkRequiredFields(requiredFieldMap: Map[FieldDescriptor, Int]): PrinterEndo = {
  ...
  def returnValue(name: String, fields: Seq[Field]): PrinterEndo = {
    _.add(s"$name(")
      .indented(
  ...
  def generateParseFrom: PrinterEndo = {
      method("parseFrom", args = Seq(Arg("`_input__`", classOf[CodedInputStream])), returnType = fp.Scala.Type(myFullScalaName))(
        varRequiredFields(requiredFieldMap) ++
        varFields(fields) ++
        parseLoop(message.fields) ++
        checkRequiredFields(requiredFieldMap) ++
        returnValue(myFullScalaName, fields)
      )
  }
}
