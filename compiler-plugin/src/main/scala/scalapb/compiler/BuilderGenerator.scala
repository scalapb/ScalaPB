...
private[compiler] class BuilderGenerator(
...
private[compiler] object BuilderGenerator {
  def generateBuilder(implicits: DescriptorImplicits, pb: ProtobufGenerator, message: Descriptor)(
      fp: FunctionalPrinter
  ): FunctionalPrinter =
    new BuilderGenerator(implicits, pb, message).generateBuilder(fp)
}
