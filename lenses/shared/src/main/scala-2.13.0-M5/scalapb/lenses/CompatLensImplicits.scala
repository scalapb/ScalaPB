...
object CompatLensImplicits {
  ...
  class SeqLikeLens[U, A, CC[_], C <: collection.SeqOps[A, CC, C]](
      val lens: Lens[U, C]
  ) extends AnyVal {
  ...
  class SetLens[U, A, CC[_], C <: collection.immutable.SetOps[A, CC, C]](val lens: Lens[U, C]) extends AnyVal {
    private def field(getter: C => A)(setter: (C, A) => C): Lens[U, A] =
      lens.compose[A](Lens[C, A](getter)(setter))
...
trait CompatLensImplicits {
  ...
  implicit def seqLikeLens[U, A, CC[_], C <: collection.SeqOps[A, CC, C]](
      lens: Lens[U, C]
  ): SeqLikeLens[U, A, CC, C] = new SeqLikeLens[U, A, CC, C](lens)

  implicit def setLens[U, A, CC[_], C <: collection.immutable.SetOps[A, CC, C]](lens: Lens[U, C]
    ): SetLens[U, A, CC,C] = new SetLens[U, A, CC, C](lens)
}
