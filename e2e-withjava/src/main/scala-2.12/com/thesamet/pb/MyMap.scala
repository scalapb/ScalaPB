...
object MyMap {
  ...
  implicit def cbf[From, K, V](implicit
      vcbf: CanBuildFrom[From, (K, V), Map[K, V]]
  ...
  class Builder[K, V] extends collection.mutable.Builder[(K, V), MyMap[K, V]] {
    ...
    def +=(other: (K, V)): this.type = { underlying += other; this }
